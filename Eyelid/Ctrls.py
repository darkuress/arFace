...
class Ctrls(Func.Func, Base.Base):
    ...
    def createLidCtrls(self, baseJnts, lEyeLoc):
        ...
        cmds.parent(self.eyelidClsGrpName, self.clsGrp)
        self.__createEyeRig()
    ...
    def __eyeLidsCrvs(self, ctlSize = 0.1):
        ...
        prefix = [self.prefix[0] + "up", self.prefix[0] + "lo" ]
        for pre in prefix:
            #- left right + upper lower prefix           
            jnts = cmds.ls(pre + "LidBlink*" + self.jntSuffix, fl =1, type="joint")
            leng = len(jnts)
            preR = pre.replace(self.prefix[0], self.prefix[1])
            #create ctrl group 
            ...
            rLidCtlGrp = cmds.duplicate(lLidCtlGrp, n = preR +"LidCtl" + self.grpSuffix)[0]
            cmds.setAttr(rLidCtlGrp + ".tx", -eyeCenterPos[0])
            cmds.setAttr(rLidCtlGrp + ".ry", -eyeRotY)
            cmds.setAttr(rLidCtlGrp + ".sx", -1)
            ...
            cmds.parent(lMinCrv,  lMaxCrv, squintCrv, annoyCrv, self.eyelidCrvGrpName) 
        ...
        for ud in self.uploPrefix:
            for lr in self.prefix:
                ...
                if not cmds.objExists(uploPrefix +"EyeOpen" + self.crvSuffix): 
                    ...
                    cmds.parent(openCrv,startCrv, self.eyelidCrvGrpName) 
                ...
                for i in range(0, length):                  
                    ...
                    cmds.parent(lidCtlP, uploPrefix + 'LidCtl' + self.grpSuffix)  
        ...
        for cn in corners:
            ...
            lidCtl, lidCtlP  = self.circleController(cn + "Lid" + self.ctlSuffix,
                                                    'xy',
                                                    ctlSize,
                                                    color = cColor,
                                                    lockAttr = ['tz',
    ...
    def __eyeCrvToJnt(self):
        self.eyeCtlExp()
        self.eyeCrvToJnt()


    def __blinkRemap(self):
        """
        """
        UDLR = [self.prefix[0] + self.uploPrefix[0], self.prefix[1] + self.uploPrefix[0]]
        for UD in UDLR:
            blinkClamp = UD[:2]+'blink_clamp'
            upLevel = self.faceFactors['eyelid'] + '.'+ UD + 'BlinkLevel'
            ...
            for i in range(0, length):
                ...
                wideSum = UD + 'WideSum' + str(i+1).zfill(2)
                DU = UD.replace(self.uploPrefix[0], self.uploPrefix[1])
                ...
                if int(cmds.about(version = True)) > 2015:
